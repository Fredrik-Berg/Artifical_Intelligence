...
if __name__ == '__main__':
    ...
    points = rng.uniform(-plane_dimensions,
                         plane_dimensions,
    ...
    noisy_z = z + rng.binomial(n=1, p=normal_prop, size=z.shape) * rng.normal(loc=0, scale=normal_std, size=z.shape)
    ...
    steps_ransac = np.empty(num_of_tests)
    steps_msac = np.empty(num_of_tests)
    steps_mlesac = np.empty(num_of_tests)
    angle_error_ransac = np.empty(num_of_tests)
    angle_error_msac = np.empty(num_of_tests)
    angle_error_mlesac = np.empty(num_of_tests)
    execution_time_ransac = np.empty(num_of_tests)
    execution_time_msac = np.empty(num_of_tests)
    execution_time_mlesac = np.empty(num_of_tests)
    ...
    for i in tqdm(range(num_of_tests)):
        ...
        best_plane_ransac, _, num_steps_ransac, time_ransac = fit_plane(pcd=pcd,
        ...
        steps_ransac[i] = num_steps_ransac
        execution_time_ransac[i] = time_ransac

        best_plane_msac, _, num_steps_msac, time_msac = fit_plane(pcd=pcd,
                                                                  confidence=confidence,
                                                                  inlier_threshold=inlier_threshold,
                                                                  min_sample_distance=min_sample_distance,
                                                                  error_func=msac_error)
        planes_msac[i] = best_plane_msac.copy()
        steps_msac[i] = num_steps_msac
        execution_time_msac[i] = time_msac

        best_plane_mlesac, best_inliers_mlesac, num_steps_mlesac, time_mlesac = \
            fit_plane(pcd=pcd,
                      confidence=confidence,
                      inlier_threshold=inlier_threshold,
                      min_sample_distance=min_sample_distance,
        ...
        steps_mlesac[i] = num_steps_mlesac
        execution_time_mlesac[i] = time_mlesac
    ...
    # Plot the execution time for the different error functions in milliseconds
    iterations_data = [execution_time_ransac * 1000.,
                       execution_time_msac * 1000.,
                       execution_time_mlesac * 1000.]
    plot_sac_comparison(iterations_data,
